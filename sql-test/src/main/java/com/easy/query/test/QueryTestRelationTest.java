...
import com.easy.query.test.doc.dto.MySignUpDTO1;
...
public class QueryTestRelationTest extends BaseTest {
    ...
    public void test2() {
        ...
        {
            List<Draft2<String, String>> list1 = easyEntityQuery.queryable(MyComUser.class)
                    .filterConfigure(NotNullOrEmptyValueFilter.DEFAULT)
                    .where(su -> {
                        su.myCompany().name().like(companyName);
                    }).select(m -> Select.DRAFT.of(
...
//                    Assert.assertEquals("1(Integer)", EasySQLUtil.sqlParameterToString(jdbcExecuteAfterArg.getBeforeArg().getSqlParameters().get(0)));
                }
                {
                    ...
                    JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArgs().get(1);
                    Assert.assertEquals("SELECT t1.`id`,t1.`time`,t1.`content`,t1.`com_id` AS `__relation__comId`,t1.`user_id` AS `__relation__userId` FROM (SELECT t.`id`,t.`com_id`,t.`user_id`,t.`time`,t.`content` FROM `my_sign_up` t WHERE t.`id` IS NOT NULL LIMIT 4) t1", jdbcExecuteAfterArg.getBeforeArg().getSql());
//                    Assert.assertEquals("1(Integer)", EasySQLUtil.sqlParameterToString(jdbcExecuteAfterArg.getBeforeArg().getSqlParameters().get(0)));
                }
                {
...
//
//
//            List<MyComUser> comUserList=new ArrayList<>();
//
//
//
//
//            List<MyComUser> collect = comUserList.stream().filter(o -> {
//                return o.getMyCompany().getName().contains("123");
//            }).collect(Collectors.toList());
//
//            List<MyComUser> list2 = easyEntityQuery.queryable(MyComUser.class)
//                    .where(m -> {
//                        m.myCompany().name().like("123");
//                    }).toList();
//
//            //------------------------
//
//            List<MyComUser> collect1 = comUserList.stream().filter(o -> {
//                return o.getMySignUps().stream().anyMatch(x -> x.getUserId().equals("123"));
//            }).collect(Collectors.toList());
//
//
//            List<MyComUser> list3 = easyEntityQuery.queryable(MyComUser.class)
//                    .where(m -> {
//                        m.mySignUps().any(x->x.userId().eq("123"));
//                    }).toList();
//            List<MySignUp> list2 = easyEntityQuery.queryable(MySignUp.class)
//                    .include(m -> m.comUser(), mq -> mq.include(x -> x.myCompany()))
...
//                    .toList();

            {

                {
                    ...
                    List<MySignUpDTO4> list1 = easyEntityQuery.queryable(MySignUp.class)
                            .where(m -> {
                                m.id().isNotNull();
//                            m.comUser().id().isNotNull();
                            }).selectAutoInclude(MySignUpDTO4.class)
                            .toList();

                    {

                        JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArgs().get(0);
                        Assert.assertEquals("SELECT t.`id`,t.`user_id`,t.`time`,t.`content`,t.`com_id` AS `__relation__comId` FROM `my_sign_up` t WHERE t.`id` IS NOT NULL", jdbcExecuteAfterArg.getBeforeArg().getSql());
//                    Assert.assertEquals("1(Integer)", EasySQLUtil.sqlParameterToString(jdbcExecuteAfterArg.getBeforeArg().getSqlParameters().get(0)));
                    }
                    {
                        JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArgs().get(1);
...
//                            m.comUser().id().isNotNull();
                        ...
                        .toList();
                for (MySignUp mySignUp : list1) {
                    if(Objects.equals(mySignUp.getComId(),"c1")){
                        Assert.assertNotNull(mySignUp.getComUserCompany());
                        Assert.assertEquals("c1", mySignUp.getComUserCompany().getId());
                    }else{
                        Assert.assertNull(mySignUp.getComUserCompany());
                    }
                }
                {

                    JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArgs().get(0);
                    Assert.assertEquals("SELECT `id`,`com_id`,`user_id`,`time`,`content` FROM `my_sign_up` WHERE `id` IS NOT NULL", jdbcExecuteAfterArg.getBeforeArg().getSql());
//                    Assert.assertEquals("1(Integer)", EasySQLUtil.sqlParameterToString(jdbcExecuteAfterArg.getBeforeArg().getSqlParameters().get(0)));
                }
                {
                    JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArgs().get(1);
...

            {
                ...
                List<MySignUpDTO3> list1 = easyEntityQuery.queryable(MySignUp.class)
                        .where(m -> {
                            m.id().isNotNull();
//                            m.comUser().id().isNotNull();
                        }).selectAutoInclude(MySignUpDTO3.class)
                        .toList();
...
//                    Assert.assertEquals("1(Integer)", EasySQLUtil.sqlParameterToString(jdbcExecuteAfterArg.getBeforeArg().getSqlParameters().get(0)));
...

            {
                ...
                List<MySignUpDTO1> list1 = easyEntityQuery.queryable(MySignUp.class)
                        .where(m -> {
                            m.id().isNotNull();
//                            m.comUser().id().isNotNull();
                        }).selectAutoInclude(MySignUpDTO1.class)
                        .toList();
