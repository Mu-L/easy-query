...
public class CTETest extends BaseTest {
    ...
    public void cteTest5() {
        ...
        Assert.assertNotNull(listenerContext.getJdbcExecuteAfterArg());
        JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArg();
        Assert.assertEquals("WITH `m8_user_temp` AS (SELECT t.`id`,t.`name` FROM `m8_user` t WHERE t.`age` IS NULL) SELECT t2.`name` AS `value1`,t10.`name` AS `value2` FROM `m8_user_temp` t2 LEFT JOIN `m8_user_temp` t5 ON t2.`name` = t5.`name` LEFT JOIN (SELECT t7.`user_id` AS `__user_id__`,t6.`id`,t6.`name`,t6.`create_time`,(ROW_NUMBER() OVER (PARTITION BY t7.`user_id`)) AS `__row__` FROM `m8_role` t6 INNER JOIN `m8_user_role` t7 ON t6.`id` = t7.`role_id`) t10 ON (t10.`__user_id__` = t5.`id` AND t10.`__row__` = ?) WHERE t10.`name` LIKE ?", jdbcExecuteAfterArg.getBeforeArg().getSql());
